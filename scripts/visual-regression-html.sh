...
const outputPath = path.join("playwright-report", "visual-report.html");
...
<div class="card">
  ...
  <div id="story-${i}" class="hidden">
    <div class="images">
      ...
      ${story.baseline
        ? `<div><p>Baseline</p><img src="../${story.baseline}"/></div>`
        : ""}

      ${story.actual
        ? `<div><p>${story.baseline ? "Actual" : "Reference"}</p>
            <img src="../${story.actual}"/></div>`
        : ""}

      ${story.diff
        ? `<div><p>Diff</p><img src="../${story.diff}"/></div>`
        : ""}
